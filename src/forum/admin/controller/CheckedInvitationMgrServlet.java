...
import forum.admin.dao.CheckedInvitationDao;
import forum.admin.dao.CheckedInvitationDaoImpl;
...
public class CheckedInvitationMgrServlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	private CheckedInvitationDao checkedInvitationDao;
	...
	public CheckedInvitationMgrServlet() {
		super();
		this.checkedInvitationDao=new CheckedInvitationDaoImpl();
		this.authorId="";
	}
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (mid == 0) {
			...
			int totalNum=this.checkedInvitationDao.getInvitaionCount(this.authorId);
			...
			this.allList=this.checkedInvitationDao.getAllInvitation(this.authorId,this.orderBy,this.order);
			
			
			this.selectAllChecked=false;
			this.checkedList=new ArrayList<Boolean>();
			for(int i=0;i<this.pageCut.getTotalNum();i++) this.checkedList.add(false);
			if(currPage==totalPage) this.limitList=this.checkedInvitationDao.getLimitInvitation(this.authorId,
					(currPage-1)*this.pageCut.getPAGESIZE(), this.pageCut.getLastNum(), this.orderBy, this.order);
			else this.limitList=this.checkedInvitationDao.getLimitInvitation(this.authorId,
			...
			request.getRequestDispatcher("/view.admin/checkedInvitationManager.jsp").forward(request,
					response);
		}
		//下一页
		else if(mid==1){
		...
		//不通过某一条帖子，机删除它
			...
			this.checkedInvitationDao.deleteInvitation(((Invitation)this.limitList.get(id)).getId());
		...
		//拒绝所有被选中的帖子
			...
			for(int i=0;i<this.checkedList.size();i++){
				if((Boolean)this.checkedList.get(i)) this.checkedInvitationDao.deleteInvitation(((Invitation)this.allList.get(i)).getId());
			}
		...
		//查看学生内容
			...
			request.setAttribute("selectAllChecked", selectAllChecked);
			request.setAttribute("checkedList", this.checkedList);
			request.getRequestDispatcher("/view.admin/checkedInvitationManager.jsp?rid=1").forward(request,
					response);
		}
		//查看帖子内容
				else if(mid==10){
					...
					request.setAttribute("selectAllChecked", selectAllChecked);
					request.setAttribute("checkedList", this.checkedList);
					request.getRequestDispatcher("/view.admin/checkedInvitationManager.jsp?rid=0").forward(request,
							response);
				}
