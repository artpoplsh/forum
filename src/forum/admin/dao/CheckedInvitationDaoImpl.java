...
public class CheckedInvitationDaoImpl implements CheckedInvitationDao {

	private Dbutil dbutil;
	private static String SQL_QUERY="select * from invitation where isChecked=1 order by ";
	private static String SQL_LIMIT_QUERY="select * from invitation where isChecked=1 order by limit ?,?";
	...
	public CheckedInvitationDaoImpl()
	{
	...
	public List getLimitInvitation(String authorid,int from, int size, String orderBy,String order) {
		...
		String sql="";
		if(authorid.equals("")){
			sql="select * from invitation where isChecked=1 order by ";
		}
		else sql="select * from invitation where isChecked=1 and authorid='"+authorid+"' order by ";
		try{
	...
	public Invitation getInvitationById(int id) {
		...
		ResultSet res=null;
		List list=null;
		String sql="select * from invitation where isChecked=1 and id=?";
		try{
			conn=this.dbutil.getConn();
			//执行查询
	...
	public int getInvitaionCount(String authorid) {
		...
		String sql="";
		if(authorid.equals("")){
			sql="select count(*) count from invitation where isChecked=1 ";
		}
		else sql="select count(*) count from invitation where isChecked=1 and authorid='"+authorid+"' ";
		try{
			conn=this.dbutil.getConn();
			//执行查询
	...
	public void deleteInvitation(int id) {
		...
		try{
			...
			invitationContentDaoImpl.deleteInvitationContentByInvitationId(id);
