...
public class StudentDownloadServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String downFileName=new String(request.getParameter("fileName").getBytes("iso8859-1"),"UTF-8");
		File f = new File(getServletContext().getRealPath("/uploadMaterial")+"/");
    	f.mkdir();
		InputStream inputStream = new FileInputStream(new File(this.getServletConfig().getServletContext().getRealPath("/uploadMaterial")+"/"+downFileName));
		//System.out.println(this.getServletConfig().getServletContext().getRealPath("/material")+"\\"+downFileName);
