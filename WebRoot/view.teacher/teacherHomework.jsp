    ...
    <div class="container theme-showcase" role="main">
      ...
      <div class="jumbotron">
       <div style="margin-top: 9px; height: 10px;">
        <h3  style="display:inline ">课程作业列表</h3>
         ...
         window.onload=function()
{
  ...
  {
    if(confirm("删除作业会同时删除这次学生上传的作业文件，是否删除?")){return true;}
    return false;
  }
} 
   </script>
      <div >
          <table class="table table-striped">
                <%
                    ArrayList<Releasehomework> list = (ArrayList<Releasehomework>)request.getAttribute("result");
                    int size = list.size();
                    Releasehomework rh = new Releasehomework();
                    %>
                 <tr><td >标题</td><td>发布日期</td><td>截止日期</td><td>操作</td></tr>
                  <%
                  for(int i = 0;i< size ;i++){
                  rh = list.get(i);
                  %>
                  <tr>
                      <td ><a href="<%=rh.getContent() %>/<%= rh.getTitle()%>"> <%=rh.getTitle() %></a></td>
                      <td><%=rh.getReleasedate() %></td>
                      <td><%=rh.getDeadline() %></td>
                  
                      <td>
                          <a href="TeacherServlet?mid=6&id=<%=rh.getId() %>" class="btn btn-success">上交情况 </a>
                          <a id="del" href='TeacherServlet?mid=5&id=<%=rh.getId() %>'  class="btn btn-danger" >删除</a> 
                      </td>
                  </tr>
